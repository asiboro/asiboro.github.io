(function() {
	...
	var z = function(a) {
		...
		B = function() {
			var a = A,
				b = "317150500 317150501 317150502 317150503 317150504 317150505".split(" ");
				...
				r: {
					if (!(1E-4 > Math.random())) {
						...
						if (d < k) {
							...
							b = b[Math.floor(d * b.length)];
							break r
						}
					}
					b = null
				}
				b && "" != b && a.a.hasOwnProperty(1) && (a.a[1] = b)
			}
		};
	...
	var A, D = "google_conversion_id google_conversion_format google_conversion_type google_conversion_order_id google_conversion_language google_conversion_value google_conversion_currency_code google_conversion_domain google_conversion_label google_conversion_color google_disable_viewthrough google_remarketing_only google_remarketing_for_search google_conversion_items google_custom_params google_conversion_date google_conversion_time google_conversion_js_version onload_callback opt_image_generator google_is_call google_conversion_page_url".split(" ");
	...
	function K(a) {
		...
		for (var b = [], d = 0, c = a.length; d < c; d++) {
			var e = a[d],
				g = [];
			e && (g.push(J(e.value)), g.push(J(e.quantity)), g.push(J(e.item_id)), g.push(J(e.adwords_grouping)), g.push(J(e.sku)), b.push("(" + g.join("*") + ")"))
		}
	...
	function L(a, b, d) {
		...
		if (a) {
			var e = a.screen;
			e && (c.push(F("u_h", e.height)), c.push(F("u_w", e.width)), c.push(F("u_ah", e.availHeight)), c.push(F("u_aw", e.availWidth)), c.push(F("u_cd", e.colorDepth)));
			a.history && c.push(F("u_his", a.history.length))
		}
		d && "function" == typeof d.getTimezoneOffset && c.push(F("u_tz", -d.getTimezoneOffset()));
		b && ("function" == typeof b.javaEnabled && c.push(F("u_java", b.javaEnabled())), b.plugins && c.push(F("u_nplug", b.plugins.length)), b.mimeTypes && c.push(F("u_nmime", b.mimeTypes.length)));
		return c.join("")
	}
	function M(a, b, d) {
		var c = "";
		if (b) {
			var c = c + F("ref", null != b.referrer ? b.referrer.toString().substring(0, 256) : ""),
				e;
			if (a.top == a) e = 0;
			else if (b = a.location.ancestorOrigins) e = b[b.length - 1] == a.location.origin ? 1 : 2;
			else {
				b = a.top;
				try {
					e = !! b.location.href || "" === b.location.href
				} catch (g) {
					e = !1
				}
				e = e ? 1 : 2
			...
			b = "";
			b = d ? d : 1 == e ? a.top.location.href : a.location.href;
			c += F("url", null != b ? b.toString().substring(0, 256) : "");
			c += F("frm", e)
		}
		return c
	}
	...
	function P(a, b, d, c) {
		...
		var e = N(a) + "//" + O(c) + "/pagead/" + [c.google_remarketing_only ? "viewthroughconversion/" : "conversion/", E(c.google_conversion_id), e, "random=", E(c.google_conversion_time)].join(""),
			g;
		r: {
			g = c.google_conversion_language;
			if (null != g) {
				g = g.toString();
				if (2 == g.length) {
					g = F("hl", g);
				...
				if (5 == g.length) {
					g = F("hl", g.substring(0, 2)) + F("gl", g.substring(3, 5));
					break r
				}
			}
			g = ""
		}
		return e += [F("cv", c.google_conversion_js_version), F("fst", c.google_conversion_first_time), F("num", c.google_conversion_snippets), F("fmt", c.google_conversion_format), F("value", c.google_conversion_value), F("currency_code", c.google_conversion_currency_code), F("label", c.google_conversion_label), F("oid", c.google_conversion_order_id), F("bg", c.google_conversion_color), g, F("guid", "ON"), F("disvt", c.google_disable_viewthrough), F("is_call", c.google_is_call), K(c), L(a, b, c.google_conversion_date), M(a, d, c.google_conversion_page_url), I(c), c.google_remarketing_for_search && !c.google_conversion_domain ? "&srr=n" : "", F("eid", C())].join("")
	}
	function Q() {
		var a = R,
			b = S,
			d = N(a) + "//www.google.com/ads/user-lists/" + [E(b.google_conversion_id), "/?random=", Math.floor(1E9 * Math.random())].join("");
		return d += [F("label", b.google_conversion_label), F("fmt", "3"), M(a, document, b.google_conversion_page_url)].join("")
	}
	...
	function T() {
		var a = R,
			...
			d = document,
			c = R;
		3 != c.google_conversion_format || c.google_remarketing_only || c.google_conversion_domain || A && B();
		var e = A ? A.c(1) : "",
			b = P(a, b, d, c),
			d = function(a, b, c) {
				return '<img height="' + c + '" width="' + b + '" border="0" alt="" src="' + a + '" />'
			};
		return 0 == c.google_conversion_format && null == c.google_conversion_domain ? '<a href="' + (N(a) + "//services.google.com/sitestats/" + ({
			ar: 1,
			bg: 1,
			cs: 1,
			da: 1,
	...
	var R = window;
	if (R) if (null != /[\?&;]google_debug/.exec(document.URL)) {
		var aa = R,
