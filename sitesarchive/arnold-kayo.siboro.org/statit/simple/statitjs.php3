...
if($st_suche[$st_sdomain]){
  if(eregi("&$st_suche[$st_sdomain]",$st_refer)){
    $st_ssuche = explode ("&$st_suche[$st_sdomain]", $st_refer);
    $st_sok=1;
  }
  else if(eregi("\?$st_suche[$st_sdomain]",$st_refer)){
    $st_ssuche = explode ("?$st_suche[$st_sdomain]", $st_refer);
    $st_sok=1;
  }
  if($st_sok==1){
    ...
    $st_unix = array("%22","%23","%24","%25","%26","%27","%2a","%2b","%2c","%5c");
    while(list($st_keyunix,$st_valunix)=each($st_unix)) {$st_ssuche = str_replace ($st_valunix, "", $st_ssuche);}
    $st_ssuche = str_replace ("+", " ", $st_ssuche);
    $st_ssuche = rawurldecode ($st_ssuche);
    if($st_ssuche){st_infile($st_search,$st_ssuche);}
  }
}
}
?>